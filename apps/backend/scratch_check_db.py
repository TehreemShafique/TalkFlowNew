...
async def check():
    ...
    async with engine.connect() as conn:
        res_jobs = await conn.execute(text("SELECT id, file_name, total_rows, created_at FROM lead_import_jobs"))
        ...
        res_leads = await conn.execute(text("SELECT count(*), source FROM leads GROUP BY source"))
