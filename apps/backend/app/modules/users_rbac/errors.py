...
register_error(
    "user.role_not_assignable", 400, "Role cannot be assigned during approval."
)
...
register_error(
    "user.master_restricted",
    403,
    "Only a MASTER_ADMIN can grant the MASTER_ADMIN role.",
)
register_error(
    "user.master_removal",
    400,
    "Cannot remove the MASTER_ADMIN role from an administrator account.",
)
register_error(
    "user.single_master", 400, "Another account already holds the MASTER_ADMIN role."
)
